...
describe 'ddclient' do
  ...
  describe 'Test standard installation' do
    ...
    it { should contain_file('ddclient.conf').with_ensure('present') }
  end
  ...
  describe 'Test standard installation with monitoring and firewalling' do
    ...
    it { should contain_firewall('ddclient_tcp_42').with_enable('true') }
  end
  ...
  describe 'Test decommissioning - absent' do
    ...
    it 'should remove ddclient configuration file' do should contain_file('ddclient.conf').with_ensure('absent') end
    it { should contain_monitor__process('ddclient_process').with_enable('false') }
    it { should contain_firewall('ddclient_tcp_42').with_enable('false') }
  end
  ...
  describe 'Test noops mode' do
    ...
    it { should contain_file('ddclient.conf').with_noop('true') }
    it { should contain_monitor__process('ddclient_process').with_noop('true') }
    it { should contain_monitor__process('ddclient_process').with_noop('true') }
    it { should contain_monitor__port('ddclient_tcp_42').with_noop('true') }
    it { should contain_firewall('ddclient_tcp_42').with_noop('true') }
  end

  describe 'Test customizations - template' do
    let(:params) { {:template => "ddclient/spec.erb" , :options => { 'opt_a' => 'value_a' } } }
    it 'should generate a valid template' do
      content = catalogue.resource('file', 'ddclient.conf').send(:parameters)[:content]
  ...
  describe 'Test customizations - source_dir' do
    let(:params) { {:source_dir => "puppet:///modules/ddclient/dir/spec" , :source_dir_purge => true } }
    it { should contain_file('ddclient.dir').with_source('puppet:///modules/ddclient/dir/spec') }
    it { should contain_file('ddclient.dir').with_purge('true') }
    it { should contain_file('ddclient.dir').with_force('true') }
  end
  ...
  describe 'Test Firewall Tools Integration' do
    ...
    it { should contain_firewall('ddclient_tcp_42').with_tool('iptables') }
  end
